...
if(!isset($_POST["submit"])){
    $sql="SELECT * FROM porduct WHERE ID='{$_GET['id']}'";
    $result= mysqli_query($con, $sql);
    $record= mysqli_fetch_array($result);
}
else{
    $sql2="SELECT * FROM porduct WHERE ID='{$_POST['id']}'";
    $result2= mysqli_query($con, $sql2);
    $rec= mysqli_fetch_array($result2);
    $title=$_POST['title'];
    if(isset($_POST['image'])){
        $target="./porduct/".basename($_FILES['image']['name']);
    }
    else{
        $target=$rec['image'];
    }
    $sql1="UPDATE porduct SET title='{$title}',image='{$target}' WHERE id='{$_POST['id']}'";
